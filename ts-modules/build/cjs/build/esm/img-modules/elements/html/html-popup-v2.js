...
exports.HtmlPopupV2 = void 0;
class HtmlPopupV2 {
    constructor(element, description) {
        this.element = element;
        this.description = description;
        this.popupElement = document.getElementById('img-dashboard_popup');
        this.data = [];
        this.create();
    }
    pop(event) {
        ...
        if (span) {
            let dataSlug = span.getAttribute('data-slug');
            if (dataSlug) {
                if (dataSlug == "first_column") {
                    dataSlug = 0;
                }
                const prop = this.data[0][dataSlug];
                if (prop) {
                    span.innerText = prop.value || prop;
                }
            }
        }
        if (div) {
        ...
        if (window.innerWidth > 700) {
            ...
            // litteral edge case
            if (leftPos > maxLeft) {
                let pielleke = document.querySelector('.pielleke');
...
exports.HtmlPopupV2 = HtmlPopupV2;
