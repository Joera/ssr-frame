...
var GroupControllerV1 = class {
  constructor(frame2, config2, index) {
    this.frame = frame2;
    this.config = config2;
    this.index = index;
...
var ElevationIcon = class {
  ...
  draw(activity) {
    ...
    this.element.style.gridColumn = "8 / span 4";
    this.element.style.gridRow = "6 / span 6";
    ...
    this.number.style.gridRow = "9 / span 3";
    this.number = this.font(this.number);
    this.number = this.center(this.number);
    this.number.innerHTML = activity.elevation_gain;
...
var MainNumbers = class {
  ...
  draw(activity) {
    ...
    this.headerElement.style.gridRow = "6 / 4";
    let h = this.window.document.createElement("h2");
    h.style.fontSize = "9.6rem";
    h.style.fontFamily = "'Inkwell Condensed A', 'Inkwell Condensed B'";
    h.style.fontStyle = "normal";
    h.style.fontWeight = 700;
    h.innerHTML = (parseInt(activity.distance) / 1e3).toString() + "km";
...
var SingleHeader = class {
  ...
  draw(activity) {
    ...
    this.headerElement.style.gridRow = "7 / 4";
    let h = this.window.document.createElement("h1");
    h.style.fontSize = "3.2rem";
    h.style.fontFamily = "'Inkwell Condensed A', 'Inkwell Condensed B'";
    h.style.fontStyle = "normal";
    h.style.fontWeight = 700;
    h.innerHTML = activity.title;
    this.headerElement.appendChild(h);
    let dateEl = this.window.document.createElement("div");
    dateEl.classList.add("activity_date");
    dateEl.innerHTML = activity.local_time;
    dateEl.style.fontSize = "3.2rem";
    dateEl.style.fontFamily = "'Inkwell Sans A', 'Inkwell Condensed B'";
    dateEl.style.fontStyle = "normal";
    dateEl.style.fontWeight = 400;
    this.headerElement.appendChild(dateEl);
    this.parent.appendChild(this.headerElement);
    return true;
  }
  redraw() {
...
var TypeIcon = class {
  ...
  draw(activity) {
    ...
    this.element.style.gridRow = "4 / 8";
...
var RunGroupV1 = class extends GroupControllerV1 {
  constructor(page, data, config2, index) {
  ...
  async init(htmlContainer) {
    ...
    this.typeIcon = new TypeIcon(groupEl, this.frame.renderer.window);
    this.header = new SingleHeader(groupEl, this.frame.renderer.window);
    this.elevationIcon = new ElevationIcon(groupEl, this.frame.renderer.window);
    this.main = new MainNumbers(groupEl, this.frame.renderer.window);
  ...
  async update(data, segment, update) {
...
var ScaleService = class {
  ...
  set(data, minValue) {
    ...
    const min2 = window.d3.min(data.filter((v) => !isNaN(v)));
    const max2 = window.d3.max(data.filter((v) => !isNaN(v)));
    switch (this.config.type) {
      ...
      case "band":
        if (self.ctrlr.config.paddingInner == void 0 || self.ctrlr.config.paddingOuter == void 0)
          return;
        this.scale = window.d3.scaleBand().domain(data).paddingInner(self.ctrlr.config.paddingInner).paddingOuter(self.ctrlr.config.paddingOuter).align(0.5);
        break;
      case "bandTime":
        this.scale = window.d3.scaleBand().domain(data).paddingInner(0.2).paddingOuter(0.5).align(0.5);
        break;
      case "normalised":
...
var GraphControllerV3 = class {
  constructor(slug, frame2, group, mapping) {
    this.slug = slug;
    this.frame = frame2;
    this.group = group;
  ...
  _addAxis(slug, scale, position, format2, extra, label) {
...
var RunV1 = class extends GraphControllerV3 {
  constructor(slug, frame2, group, mapping, segment, index) {
    super(slug, frame2, group, mapping);
    this.slug = slug;
    this.frame = frame2;
    this.group = group;
    this.mapping = mapping;
  ...
  async update(data, segment, update, range) {
